...
public class LoopText : Text
{
    ...
    private void UpdateMaskRect()
    {
        if (material.HasProperty("_MaskRect"))
        {
            ...
            //var t0 = new Vector2(pos.x, pos.y);
            var t1 = t0 - rectTransform.rect.size / 2;
            var t2 = t0 + rectTransform.rect.size / 2;

            materialForRendering.SetVector("_MaskRect", new Vector4(t1.x, t1.y, t2.x, t2.y));
            SetMaterialDirty();
        }
    }
